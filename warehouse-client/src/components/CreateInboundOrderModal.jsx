...
const emptyItem = () => ({ productName: '', barcode: '', quantity: '', minimumQuantity: '', sectorId: '' })

export default function CreateInboundOrderModal({ open, onClose, onCreated }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!supplierName.trim() || items.some(i => !i.productName.trim() || !i.sectorId)) return
    setSaving(true)
    try {
      await api.post('/inboundorders', {
        ...
        items: items.map(i => ({
          ...
          minimumQuantity: parseInt(i.minimumQuantity) || 0,
          sectorId: i.sectorId,
        })),
      })
      setSupplierName('')
      ...
      <div style={{
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div>
            ...
            <input value={supplierName} onChange={e => setSupplierName(e.target.value)} placeholder="e.g. Fresh Farm Supplies" style={inputStyle} required />
          </div>

          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
              <p style={{ ...labelStyle, marginBottom: 0 }}>Items *</p>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                    <div>
                      ...
                      <input value={item.productName} onChange={e => updateItem(index, 'productName', e.target.value)}
                        placeholder="e.g. Whole Milk" style={inputStyle} required />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <input type="number" min="0" value={item.minimumQuantity} onChange={e => updateItem(index, 'minimumQuantity', e.target.value)}
